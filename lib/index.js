...
var fs = require('fs'),
    ...
    exportFile = require('newman/lib/run/export-file.js'),
...
function getContentTypeFromHeaders (headers) {
    ...
    if (headers && headers.members) {
        networkHeaders = headers.members;
    }
    else if (headers) {
        networkHeaders = headers;
    }

    for (let i = 0; i < networkHeaders.length; i++) {
        let currentHeader = networkHeaders[i];

        if (currentHeader.key.toLowerCase() === 'content-type') {
...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    handlebars.registerHelper('prettyPrint', function (text, headers) {
        console.log('pretty print start');
        let contentType = getContentTypeFromHeaders(headers);
        let formattedContent = '';

        if (!contentType) {
            formattedContent = '--- reporter:: Content type not recognized ---';
        }
            ...
            if (jsonData) {
                ...
                formattedContent = '--- reporter:: JSON parse error ---'
            }
        }
            ...
            formattedContent = '--- reporter:: ' + contentType + ' not shown.'
        }

        return formattedContent.trim().slice(0, 10000);
    });
    ...
    // Prevent node-js exiting automatically
    process.stdin.resume();

    var isReportCreated = false;

    function exitHandler () {
        console.log(`Task is exiting!!! Exit code isReportCreated (var) is ${isReportCreated}`);

        // When report is created, all good just exit
        if (isReportCreated) {
            console.log('Report was created, exiting process!')
            process.exit();

            return;
        }

        console.log('Creating report before exit!');
        newman.emit('beforeDone');

        newman.exports.forEach(function (data) {
            exportFile(data, function (err) {
                newman.emit('done', err, newman.summary);
    ...
    newman.on('beforeDone', function () {
        ...
        this.exports.push({
            name: 'html-reporter-htmlextra-onexit',
